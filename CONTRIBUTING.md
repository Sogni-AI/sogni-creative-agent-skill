...
7. Verify: `npx -y --package @sogni-ai/sogni-creative-agent-skill@latest sogni-agent --version` and `sogni-agent doctor` on a clean machine.
8. ClawHub (publisher `fishmongr`, slug `sogni-creative-agent-skill`): stage a skill-only folder — SKILL.md, references/, skills/, llm.txt, README, LICENSE, CHANGELOG.md, skill-package.json, the CLI runtime files (`sogni-agent.mjs`, `media-utilities.mjs`, `sam-segmentation.mjs`, `sogni-app-id.mjs`, `sogni-hosted-client.mjs`, `attribution.mjs`, `env.mjs`, `ssrf-guard.mjs`, `update-check.mjs`, `node-version-check.mjs`, `version.mjs`, `generated/`, `host-launchers/`) and **no** `openclaw.plugin.json` / `openclaw-plugin.mjs` / `package.json` (their presence makes ClawHub classify the artifact as a code plugin, which requires scoped names and extra `openclaw.compat`/`openclaw.build` manifest fields). Verify every relative runtime import resolves inside that folder, then run `npx -y clawhub@latest publish <staged-dir> --owner fishmongr --slug sogni-creative-agent-skill --name "Sogni Creative Agent Skill" --version x.y.z --changelog "..."`. Publication may remain pending security scans after upload succeeds; report that state until the public version advances.
9. Run `npm run build:mcpb`, verify the extracted bundle starts, then create the GitHub release from the published tag and attach `dist/sogni-creative-agent.mcpb` with `gh release create vx.y.z dist/sogni-creative-agent.mcpb --verify-tag --title vx.y.z --notes-file <release-notes-file>`.
10. Confirm the pushed commit contains the synchronized Claude/Codex plugin manifests and self-contained Hermes bundle. Those installation surfaces load this GitHub repository; OpenClaw's code plugin loads the npm package. Check skill discovery with `npx skills add Sogni-AI/sogni-creative-agent-skill --list` and confirm ClawHub's latest version with `npx -y clawhub@latest inspect @fishmongr/sogni-creative-agent-skill`. The bare ClawHub slug is ambiguous; installation also uses `@fishmongr/sogni-creative-agent-skill`.
