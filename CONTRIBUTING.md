...
10. Confirm the pushed commit contains the synchronized Claude/Codex plugin manifests and self-contained Hermes bundle. Those installation surfaces load this GitHub repository; OpenClaw's code plugin loads the npm package. Check skill discovery with `npx skills add Sogni-AI/sogni-creative-agent-skill --list` and confirm ClawHub's latest version with `npx -y clawhub@latest inspect sogni-creative-agent-skill`.
