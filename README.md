...
- create videos from text, images, audio, or reference video (LTX-2.3, WAN 2.2, Wan Animate 2, Seedance 2.0, HappyHorse 1.1)
...
| `--sampler <name>` | Image/music sampler; for MiniMax H3 Turbo video only: `euler`, `er_sde`, or `sa_solver`. H3 Turbo defaults to `er_sde` on Socket; the CLI omits this field unless explicitly set. |
...
| Native image + driving-video motion transfer | `wan-animate-2` with `--workflow animate-move` |
...
- **Wan Animate 2** requires both an original reference still and a raw driving video, runs at fixed 24 fps for 17-81 frames (`1+n×4`), preserves source audio, and uses the distilled model's fixed 10-step, guidance-1, shift-5, `euler`/`simple` recipe. The optional `--enable-context-window` branch uses 21-frame windows with 8-frame overlap and defaults off for full-sequence release quality. Production routing requires a 32 GB-class worker at every supported resolution. The still must come directly from an image model—not a screenshot or extracted video frame—for release-quality validation. Keep WAN 2.2 as the default until Animate 2 output receives manual quality approval.
