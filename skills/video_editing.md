...
- Use `upscale_video` when the user only wants a sharper, higher-resolution copy of an existing video. Pick the source with `sourceVideoIndex` (generated results are 0-based; -1 is the first upload) and optionally `targetResolution` 1080 or 1440 (default 1440, or 1080 for sources under 720p). It cannot produce 4K. Sources must be ≤768px on the short edge and at most about 1344×768 pixels (768×1344 portrait), 1-60 fps, and ≤100 MB. The server sets the maximum clip length; relay its error if a source is too long, without quoting a fixed limit. Never invent a prompt or substitute `video_to_video`/`generate_video` unless the user names a generative model such as Seedance for a re-render.
